...
const argumentsToString = (v) => {
    ...
    let i = 0;
    for (var k in args) {
        if ((i === 0) && (typeof args[k] === 'object')) {
            ++i;
            args[k] = args[k].constructor.name;
            continue;
        }
...
class Logger {
    ...
    constructor(config) {
        ...
        var welcome = () => {
            ...
            this._logger.log("info", LOG_ID + " OS " + os.type());
            this._logger.log("info", LOG_ID + "Welcome to the " + this.colors.magenta("ALE Rainbow S2S Dev kit for Node.JS") + "");
        ...
        if (enableFileLog) {
            if (("logs" in config) && ("file" in config.logs) && ("path" in config.logs.file)) {
                if (os.type().indexOf('Windows') >= 0) {
                    logDir = `${process.env.APPDATA}${path.sep}rainbow-s2s-sdk`;
                } else {
